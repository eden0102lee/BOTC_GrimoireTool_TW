...
function main() {
  ...
  // 4) TB patches (from apply-tb-skill-cards-to-rules.js)
  const tb = require("./apply-tb-skill-cards-to-rules.js");
  Object.keys(tb.PATCHES).forEach((id) => {
    const idx = byId.get(id);
    if (idx == null) return;
    const before = JSON.stringify(data.rules[idx]);
    data.rules[idx] = tb.deepMergeRule(data.rules[idx], tb.PATCHES[id]);
    if (JSON.stringify(data.rules[idx]) !== before) updated.push(id);
  });
  tb.NEW_RULES.forEach((rule) => {
    const id = String(rule.id).toLowerCase();
    if (byId.has(id)) {
      const idx = byId.get(id);
      data.rules[idx] = tb.deepMergeRule(data.rules[idx], rule);
      updated.push(id);
      return;
    }
