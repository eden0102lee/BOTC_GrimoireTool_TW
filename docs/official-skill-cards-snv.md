...
- 動詞: 殺害／轉化
- 輸入: （夜殺／轉化）alivePlayer:target
- effects: （夜殺）目標死亡；（轉化）自身死亡、目標變方古、目標改邪惡
- template: （見雙卡）
- 例句:
...
- 動詞: (無)
- 輸入: （首夜）player:p1(鏡像雙子), player:p2(對立雙子)；（觸發）select:res(結果)
- effects: 雙方 雙胞胎
- template: `{p1} 與 {p2} 互認`／`{actor} → 觸發 → {res}`
- 例句:
...
- 觸發: (無)
- 動詞: 選擇
- 輸入: alivePlayer:p1(目標玩家), role:r1(對應角色)
- effects: 目標變成該角色（角色不在場）；（可選）當晚死亡—說書人選
...
└ 說書人選擇 小美【02.方古】死亡
```
- 備註: 角色在場則關 setRole；變出新惡魔時當晚死亡由說書人決定
...
- 觸發: (無)
- 動詞: 殺害
- 輸入: （setup）player:p1/p2（鄰近鎮民）；（夜殺）alivePlayer:target
- effects: （setup）兩側鎮民中毒；（夜殺）目標死亡
- template: （見雙卡）
- 例句:
...
└ 小明【03.藝術家】死亡
```
- 備註: setup 時對兩側最近鎮民掛中毒（座位／角色變動時需更新）
...
- 觸發: (無)
- 動詞: 殺害
- 輸入: alivePlayer:target(擊殺目標), player:poisoned(鄰近中毒鎮民（可空）)
- effects: 目標死亡；（殺爪牙）目標具有能力、鄰近鎮民中毒
...
- 動詞: 猜測／得知
- 輸入: text:guess(公開猜測（最多五次）)；number:num(得知數字)
- effects: 自身 正確
- template: `{actor} → 首日公開猜測 → {guess}`／`{actor} → {action} → {num}`
- 例句:
