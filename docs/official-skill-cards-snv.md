...
- 動詞: 選擇
- 輸入: alivePlayer:target(選擇的存活玩家), select:align(落敗陣營)
- effects: (無)（└ 陣營落敗為紀錄句，非魔典標記）
...
- 動詞: 殺害
- 輸入: 主卡 player:target(目標對象)；轉化時可選效果（自身死亡／目標成邪惡方古）；setup 無輸入
- effects: 主卡：目標死亡、目標掛死亡標記、可選「第一位外來者」；轉化可選：自身死亡、目標變邪惡方古；setup：本局外來者 +1
- template: 主卡 `{actor} → {action} → 選擇 {target} → {target} 死亡`；setup `本局外來者 +1`
- 例句:
...
└ 小美【04.呆瓜】成為邪惡方古
```
- 備註: 多卡：setup(+1)／夜殺／外來者轉化（每局一次，可選 effects）
...
- 觸發: 隔日被詛咒者發起提名 → 死亡
- 動詞: 詛咒
- 輸入: 主卡 player:target(目標對象)；觸發沿用被詛咒玩家（或 player:target）
- effects: 主卡：目標對象 詛咒；觸發：目標死亡
- template: 主卡 `{actor} → {action} → 選擇 {target}`；觸發 `{target}被咒殺死亡`
- 例句:
...
- 觸發: 白天首次處決後可再提名一人
- 動詞: 追加提名
- 輸入: alivePlayer:target(被追加提名的玩家)
- effects: (無)
...
- 觸發: (無)
- 動詞: 殺害
- 輸入: 主卡 player:target(目標對象), player:p1(鄰近中毒鎮民，可空)；setup 無輸入
...
└ 小美【04.藝術家】中毒
```
- 備註: 多卡：setup(−1)／夜殺；殺爪牙→保留能力＋鄰近一鎮民中毒
...
- effects: 可選互換角色／陣營；目標對象 中毒（新舞蛇人）
- template: `{actor} → {action} → 選擇 {target}`
- 例句:
