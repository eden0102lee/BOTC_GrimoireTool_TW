...
- 備註: 夜殺卡為 optional（僅白日無人死時用）；假死另卡
...
- 觸發: 當日有外來者死亡 → 當晚可殺一人
- 動詞: 得知／殺害
- 輸入: （setup）select:res(−1/+1)；（首夜）text:note(場上外來者)；（觸發夜）alivePlayer:target(擊殺目標)
...
- 備註: 首夜資訊；殺人僅在外來者白日死後（optional 卡）
...
- 動詞: (無)
- 輸入: player:target(首位選中莽夫的玩家)
- effects: 選中者 醉酒、自身 轉為選中者陣營
